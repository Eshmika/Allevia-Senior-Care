...
function doGet(e) {
  ...
  } else if (e.parameter.page === "client-sign-exhibit-a" && e.parameter.id) {
    // Serve Exhibit A for clients
    var clientDetails = getClientDetails(e.parameter.id);
    if (clientDetails) {
      var template = HtmlService.createTemplateFromFile("Exhibit-A");
      template.clientId = e.parameter.id;
      template.clientData = clientDetails;
      template.scriptUrl = ScriptApp.getService().getUrl();
      template.isPdf = false;
      ...
      return template
        .evaluate()
        .setTitle("Exhibit A Cost of Services - Allevia Senior Care")
        .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL)
        .addMetaTag("viewport", "width=device-width, initial-scale=1");
    } else {
      return HtmlService.createHtmlOutput(
  ...
  } else if (e.parameter.page === "client-sign-exhibit-b" && e.parameter.id) {
    // Serve Exhibit B for clients
    var clientDetails = getClientDetails(e.parameter.id);
    if (clientDetails) {
      var template = HtmlService.createTemplateFromFile("Exhibit-B");
      template.clientId = e.parameter.id;
      template.clientData = clientDetails;
      template.scriptUrl = ScriptApp.getService().getUrl();
      template.isPdf = false;
      ...
      return template
        .evaluate()
        .setTitle("Exhibit B Plan of Care - Allevia Senior Care")
        .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL)
        .addMetaTag("viewport", "width=device-width, initial-scale=1");
    } else {
      return HtmlService.createHtmlOutput(
  ...
  } else if (e.parameter.page === "client-sign-hipaa" && e.parameter.id) {
    // Serve HIPAA Release for clients
    var clientDetails = getClientDetails(e.parameter.id);
    if (clientDetails) {
      var template = HtmlService.createTemplateFromFile("HIPAA-Release");
      template.clientId = e.parameter.id;
      template.clientData = clientDetails;
      template.scriptUrl = ScriptApp.getService().getUrl();
      template.isPdf = false;
      ...
      return template
        .evaluate()
        .setTitle("HIPAA Authorization - Allevia Senior Care")
        .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL)
        .addMetaTag("viewport", "width=device-width, initial-scale=1");
    } else {
      return HtmlService.createHtmlOutput(
...
// Handler for Exhibit A submission: generates PDF, uploads to Drive, saves link
function submitClientExhibitA(form) {
  try {
    var id = form.clientId;
    var signature = form.signature;
    var signDate = form.signDate;
    ...
    var pdfBlob = template
      ...
      .getAs(MimeType.PDF)
      .setName(
        `${details.firstName} ${details.lastName} - Exhibit A Cost of Services.pdf`
      );
    ...
    // 6. Save Link to Sheet
    var fileUrl = file.getUrl();
    var saved = saveClientDocumentLink(id, "exhibitA", fileUrl);
...
// Handler for Exhibit B submission: generates PDF, uploads to Drive, saves link
function submitClientExhibitB(form) {
  try {
    var id = form.clientId;
    var signature = form.signature;
    var signDate = form.signDate;
    ...
    var pdfBlob = template
      ...
      .getAs(MimeType.PDF)
      .setName(
        `${details.firstName} ${details.lastName} - Exhibit B Plan of Care.pdf`
      );
    ...
    // 6. Save Link to Sheet
    var fileUrl = file.getUrl();
    var saved = saveClientDocumentLink(id, "exhibitB", fileUrl);
...
// Handler for HIPAA Release submission: generates PDF, uploads to Drive, saves link
function submitHipaaRelease(form) {
  try {
    var id = form.clientId;
    var signature = form.signature;
    var signDate = form.signDate;
    ...
    var pdfBlob = template
      ...
      .addMetaTag("viewport", "width=device-width, initial-scale=1")
      .getAs(MimeType.PDF)
      .setName(`${details.firstName} ${details.lastName} - HIPAA Release.pdf`);
    ...
    // 6. Save Link to Sheet
    var fileUrl = file.getUrl();
    var saved = saveClientDocumentLink(id, "hipaaRelease", fileUrl);
...
function saveClientDocumentLink(clientId, docType, fileUrl) {
  try {
    ...
    } else if (docType === "exhibitA") {
      colName = "Exhibit A Link";
    } else if (docType === "exhibitB") {
      colName = "Exhibit B Link";
